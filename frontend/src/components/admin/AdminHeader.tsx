...
export default function AdminHeader({ 
  ...
  return (
    <header className="h-[72px] bg-white dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 flex items-center justify-between px-4 sm:px-8 z-30 sticky top-0 transition-colors">
      ...
      <div className="flex items-center gap-3 sm:gap-5">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 py-1.5 animate-in fade-in slide-in-from-top-4 z-50">
              <div className="px-4 py-3 border-b border-slate-50 dark:border-slate-700 sm:hidden">
                <div className="text-sm font-bold text-slate-800 dark:text-slate-100">{userName}</div>
                <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">{userRole}</div>
              </div>
              <div className="p-1.5">
                <button 
                  onClick={() => {
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-indigo-650 dark:hover:text-indigo-400 rounded-xl transition-colors"
                >
                  <User className="w-4 h-4" /> Profil Saya
                </button>
                <div className="h-px bg-slate-50 dark:bg-slate-800 my-1.5 mx-2"></div>
                <button 
                  onClick={handleLogout}
                  className="w-full flex items-center gap-3 px-3 py-2 text-sm font-semibold text-red-600 dark:text-red-455 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors"
                >
