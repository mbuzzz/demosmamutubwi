      ...
      <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-none z-50 safe-area-pb pb-2 px-2 pt-2 flex justify-between items-center transition-colors">
        {mainTabs.map((tab) => {
          ...
          return (
            ...
            >
              <div className={`p-1.5 rounded-xl transition-all ${isActive ? 'bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] animate-in fade-in duration-200">
          ...
          <div className="absolute bottom-0 left-0 w-full bg-white dark:bg-slate-900 rounded-t-3xl shadow-2xl animate-in slide-in-from-bottom-full duration-300 max-h-[85vh] overflow-hidden flex flex-col transition-colors">
            ...
            <div className="p-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
              <h3 className="font-extrabold text-slate-800 dark:text-white text-lg">Menu Lainnya</h3>
              <button 
                onClick={() => setShowMore(false)}
                className="p-2 bg-white dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
