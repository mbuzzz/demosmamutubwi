    ...
    <div className="bg-slate-50 py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-extrabold text-slate-900 mb-3">Direktori Guru & Pendidik</h1>
          <p className="text-slate-500">Mata rantai utama transfer keilmuan dan kepribadian Islami bagi siswa kami.</p>
        </div>
            ...
            {filteredTeachers.map((teacher, index) => (
              <div key={index} className="bg-white rounded-xl p-6 shadow-sm border hover:shadow-md transition-shadow flex items-start gap-4">
                ...
                <div className="space-y-1">
                  <h3 className="font-bold text-slate-900 text-base leading-snug">{teacher.name}</h3>
                  <div className="flex items-center gap-1 text-emerald-700 text-xs font-semibold uppercase tracking-wider">
                    <BookOpen className="h-3 w-3" />
                    <span>{teacher.subject}</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-slate-500 text-xs mt-2 pt-1 border-t border-slate-100">
