...
export default function LoginOrangTua() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('remember_ortu_username');
    if (saved) {
      setUsername(saved);
      setRememberMe(true);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.role !== 'orang_tua') {
        throw new Error('Akun Anda bukan akun Orang Tua / Wali Murid. Silakan gunakan portal yang sesuai.');
      }

      // Save or remove remembered username
      if (rememberMe) {
        localStorage.setItem('remember_ortu_username', username);
      } else {
        localStorage.removeItem('remember_ortu_username');
      }

      setSimulatedRole(user.role as Role);
      navigate('/panel/siswa');
      toast.success(`Selamat datang di Portal Orang Tua, ${user.name}!`);
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden transition-colors">
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl border border-slate-200 dark:border-slate-800 p-8 relative z-10">
        <div className="text-center mb-6">
          <Link to="/login" className="inline-flex items-center gap-1.5 text-slate-500 hover:text-indigo-600 transition-colors font-bold text-xs mb-4">
          ...
          <h2 className="text-2xl font-black text-slate-800 dark:text-white">Portal Orang Tua</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Masukkan username wali murid dan password Anda</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1.5 pl-1">Username Orang Tua</label>
          ...
          >
            <LogIn className="w-4 h-4" /> Masuk Portal Orang Tua
          </button>
        </form>
      </div>
    </div>
