...
import { School, Target, Quote, Save, Loader2, Image as ImageIcon } from 'lucide-react';
...
export default function AdminProfilSekolah() {
  ...
  useEffect(() => {
    if (profil) {
      setForm({
        ...
        misi_text: profil.misi_list ? profil.misi_list.join('\n') : '',
  ...
  const handleSaveSejarah = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleSaveVisiMisi = async (e: React.FormEvent) => {
    ...
    const misiArray = form.misi_text.split('\n').map(m => m.trim()).filter(Boolean);
    payload.append('misi_list', JSON.stringify(misiArray));
    await updateProfil.mutateAsync(payload);
  };

  const handleSaveSambutan = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = new FormData();
      ...
      <div className="space-y-6">
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[15px] border border-slate-100 dark:border-slate-800 p-6">
            ...
            {activeTab === 'sejarah' && (
              <form onSubmit={handleSaveSejarah}>
                ...
                <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                  <button type="submit" disabled={updateProfil.isPending} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-colors disabled:opacity-50">
            ...
            {activeTab === 'visimisi' && (
              <form onSubmit={handleSaveVisiMisi}>
                <h3 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
            ...
            {activeTab === 'sambutan' && (
              <form onSubmit={handleSaveSambutan}>
                <h3 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
