          ...
          {activeTab === 'sejarah' && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="xl:col-span-2 space-y-4">
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">Teks Sejarah Sekolah</label>
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                  <ReactQuill theme="snow" value={sejarahContent} onChange={setSejarahContent} className="h-80 pb-10" />
                </div>
              </div>
              <div className="space-y-4">
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">Foto Utama Sejarah</label>
                <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:bg-slate-50 dark:bg-slate-800/50 transition-colors cursor-pointer bg-white dark:bg-slate-900">
                  <ImageIcon className="w-8 h-8 text-indigo-400 mx-auto mb-3" />
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Pilih Foto Lama / Gedung</p>
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">Format JPG/PNG (Landscape)</p>
                </div>
              </div>
            </div>
          )}
                ...
                <div className="space-y-3">
                  {[1, 2, 3].map((num) => (
                    <div key={num} className="flex items-start gap-3 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-200 dark:border-slate-700 group">
                      ...
                      <button className="p-2 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-lg mt-1"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="xl:col-span-1 space-y-4">
                <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">NIP / NBM</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Foto Kepala Sekolah</label>
                    <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 dark:bg-slate-800/50 transition-colors cursor-pointer">
                      <User className="w-8 h-8 text-indigo-400 mx-auto mb-2" />
                      <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Upload Foto Profil</p>
                      <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">Portrait, Maks 2MB</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="xl:col-span-2 space-y-4">
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200">Teks Isi Sambutan</label>
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
