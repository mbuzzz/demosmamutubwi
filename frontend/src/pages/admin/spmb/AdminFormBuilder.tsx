        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[15px] shadow-card p-6 border border-slate-100">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Tipe Field</label>
                <select 
                  value={fieldType}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Label Field</label>
                <input type="text" placeholder="Contoh: Nama Ayah" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>

              {fieldType === 'select' && (
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">Opsi Pilihan (Pisahkan dgn koma)</label>
                  <input type="text" placeholder="Islam, Kristen, Katolik..." className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
              )}

              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Lebar Kolom UI</label>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                ...
                <label htmlFor="req" className="text-sm font-medium text-slate-700 cursor-pointer">Wajib Diisi (Required)</label>
              </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-[15px] shadow-card overflow-hidden border border-slate-100">
            <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
              <h3 className="font-bold text-slate-800 flex items-center gap-2"><Settings className="w-4 h-4 text-slate-500"/> Urutan & Field Aktif</h3>
              <span className="text-xs font-medium px-2.5 py-1 bg-white border border-slate-200 rounded-md text-slate-500 shadow-sm">Drag & Drop untuk urutan</span>
            </div>
            
            <div className="divide-y divide-slate-100">
              {formFields.map(field => (
                <div key={field.id} className="flex items-center p-4 hover:bg-slate-50 transition-colors group bg-white">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {field.required ? (
                        ...
                        <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded font-bold uppercase tracking-wider border border-slate-200">Opsional</span>
                      )}
                    </div>
                    <div className="text-xs text-slate-500 mt-1.5 flex flex-wrap gap-x-4 gap-y-1">
                      <span className="flex items-center gap-1"><ToggleLeft className="w-3.5 h-3.5 text-slate-400" /> {field.type}</span>
                      <span>Lebar: <strong className="text-slate-600">{field.width}</strong></span>
                      {field.options && <span className="text-indigo-500 truncate max-w-[200px]">[{field.options}]</span>}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="p-1.5 bg-white border border-slate-200 shadow-sm rounded-lg text-slate-500 hover:text-indigo-600 hover:border-indigo-200"><Edit className="w-4 h-4" /></button>
                    <button className="p-1.5 bg-white border border-slate-200 shadow-sm rounded-lg text-slate-500 hover:text-red-600 hover:border-red-200"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              ))}
            </div>
