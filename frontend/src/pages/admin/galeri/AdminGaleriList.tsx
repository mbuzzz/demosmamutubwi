...
export default function AdminGaleriList() {
  ...
  return (
    <AdminLayout title="Manajemen Galeri">
      ...
      {/* Upload Area */}
      <div className="bg-white rounded-[15px] shadow-card p-6 mb-8 border border-slate-100">
        <div className="border-2 border-dashed border-indigo-200 bg-indigo-50/50 rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-indigo-50 transition-colors">
          ...
          <h3 className="font-semibold text-slate-800 text-lg">Klik atau seret foto ke sini</h3>
          <p className="text-slate-500 text-sm mt-1">Mendukung format JPG, PNG. Maksimal ukuran file 2MB.</p>
      ...
      <div className="bg-white rounded-[15px] shadow-card p-6 border border-slate-100">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-bold text-slate-800 text-lg">Foto Terpublikasi</h2>
          <span className="text-sm text-slate-500">{photos.length} Foto</span>
        </div>
        
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {photos.map(photo => (
            <div key={photo.id} className="group relative rounded-xl overflow-hidden aspect-square border border-slate-200">
